...
class TradingStrategy < ActiveRecord::Base
  ...
  def long?
    ...
    if quote_value
      ...
      log_if("Checking for #{@days_back_long_short} days back long/short #{@current_date_time}-#{(@days_back_long_short).days} at #{@current_date_time-(@days_back_long_short).days} value now #{@current_quote_value} value then #{value_then}")
        ...
        (@from_hour..@to_hour).each do |hour|
          (0..59).step(5).each do |minute|
            ...
            if population.stop_loss_enabled and @stop_loss_paused_until and @stop_loss_paused_until > @current_simulation_time and not @current_position_units
              ...
              @simulated_trading_signals_array<<{:name=>"Stop Loss Close", :current_date_time=>@current_simulation_time, :description=>"Stop Loss Closed after #{@stop_loss_pause_minutes} minutes daily P/L #{@daily_p_and_l}"}
              @stop_loss_paused_until = nil
              @daily_p_and_l = 0.0
            end
            ...
            if population.stop_loss_enabled and @daily_p_and_l<-(@stop_loss_value) and not @current_position_units
              ...
              @simulated_trading_signals_array<<{:name=>"Stop Loss Open", :current_date_time=>@current_simulation_time, :description=>"Stop Loss Open after loss of #{@stop_loss_value} for #{@stop_loss_pause_minutes} minutes daily P/L #{@daily_p_and_l}"}
            end
