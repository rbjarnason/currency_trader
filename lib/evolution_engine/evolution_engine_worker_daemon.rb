...
class EvolutionEngineWorker < BaseDaemonWorker
  ...
  def poll_for_trading_strategy_set_work
    ...
    @trading_strategy_set = TradingStrategySet.where(:active => true, :in_process => false, :complete => false, :error_flag => false).first
    if @trading_strategy_set
  ...
  def process_population_target
    puts "process_population_target before evolve"
    @population.evolve
    puts "process_population_target after evolve"
  ...
  def poll_for_evolution_work
    ...
    @population = TradingStrategyPopulation.where(:active => true, :complete => false, :in_process => true).first
